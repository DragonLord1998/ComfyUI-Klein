#!/bin/bash
# ============================================================================
# ComfyUI Startup Script with Flux 2 Klein 9B & LTX-2 Model Downloads
# Based on runpod-workers/comfyui-base
# ============================================================================
...
export_env_vars() {
    ...
    printenv | grep -E '^RUNPOD_|^PATH=|^_=|^CUDA|^LD_LIBRARY_PATH|^PYTHONPATH|^HF_TOKEN' | while read -r line; do
...
download_models() {
    echo ""
    echo "=============================================="
    echo "  Downloading Flux 2 Klein 9B & LTX-2 Models"
    echo "=============================================="
    echo ""
    
    # Check if models already exist
    MODELS_DOWNLOADED_FLAG="/workspace/runpod-slim/.models_downloaded"
    
    if [ -f "$MODELS_DOWNLOADED_FLAG" ]; then
        echo "Models already downloaded (flag file exists). Skipping..."
        echo "To re-download, delete: $MODELS_DOWNLOADED_FLAG"
        return 0
    fi
    ...
    if [ $DOWNLOAD_STATUS -eq 0 ]; then
        ...
        echo "Warning: Some models may not have been downloaded."
...
if [ ! -d "$COMFYUI_DIR" ] || [ ! -d "$VENV_DIR" ]; then
    ...
    for node_dir in */; do
        if [ -d "$node_dir" ]; then
            ...
            if [ -f "requirements.txt" ]; then
                echo "Installing requirements.txt for $node_dir"
                pip install --no-cache -r requirements.txt 2>/dev/null || pip install --no-cache-dir -r requirements.txt
            fi
            ...
            if [ -f "setup.py" ]; then
                echo "Running setup.py for $node_dir"
                pip install --no-cache -e . 2>/dev/null || pip install --no-cache-dir -e .
            fi
        fi
    done
fi

# Download models after ComfyUI setup
download_models
